# Olá, seja bem-vindo!

**Analista de Dados** | **Engenheiro de Dados Jr**

Me chamo Guilherme Miguel, estou cursando Ciência da Computação e em busca da minha primeira experiência como Analista de Dados ou Engenheiro de Dados Jr (estágio ou júnior).

Trabalho com Python (Pandas, NumPy), SQL/PostgreSQL e construção de pipelines de dados — do tratamento e carga em banco até automação de processos corporativos.

Portfólio completo: https://guilherme-miguel9.github.io/portfolio/

LinkedIn: https://www.linkedin.com/in/guilherme-miguel-dados

---

### Projetos em Destaque

- **[Lumi — App de Integração & Sincronização SQL](https://github.com/guilherme-miguel9/APP-INTEGRACAO-BD)** — App desktop (PySide6) que valida e carrega planilhas Excel no PostgreSQL via `COPY`.
- **[Bot Auditor de Registros Operacionais](https://github.com/guilherme-miguel9/Bot-Auditor)** — Automação de auditoria que reduz um processo de ~9h para 2-5 min.
- **[RAG-PDF](https://github.com/guilherme-miguel9/RAG-PDF)** — Assistente RAG 100% local para consulta de documentos técnicos, com citação de página.

---

### Linguagens e Tecnologias
<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original-wordmark.svg" height="50" alt="python" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original.svg" height="50" alt="postgresql" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/pandas/pandas-original.svg" height="50" alt="pandas" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/numpy/numpy-original.svg" height="50" alt="numpy" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original.svg" height="50" alt="docker" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg" height="50" alt="javascript" />
  <img width="8" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg" height="50" alt="git" />
</div>

<sub>Docker: usado para bancos de dados temporários em ambiente de desenvolvimento. JavaScript: usado principalmente em automações no Google Apps Script.</sub>

---

<div align="center">
  <img src="https://streak-stats.demolab.com?user=guilherme-miguel9&theme=dark&hide_border=true&background=0D1117&ring=ffa107&fire=ffa107&currStreakLabel=ffa107" alt="GitHub Streak" />
</div>

---

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/guilherme-miguel9/guilherme-miguel9/output/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/guilherme-miguel9/guilherme-miguel9/output/pacman-contribution-graph.svg">
  <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/guilherme-miguel9/guilherme-miguel9/output/pacman-contribution-graph.svg">
</picture>
